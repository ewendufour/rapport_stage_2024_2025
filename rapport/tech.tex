...
\end{align*}
\end{definition}
\begin{definition}[Contextes d'application]
\begin{align*}
...
\begin{lemma}[Préservation des substitutions]
  Soient $\mathcal{C}$ un contexte de substitution, $x$ et $y$ des variables et t un terme.
  \[
...
\begin{lemma}[Propreté des substitutions]
  \label{prls}
 Soient $\mathcal{C}$ un contexte de substitution, $t$ un terme et $x$ une variable.
...
\begin{lemma}
  \label{lemma0}
  Soient $\mathcal{L}$ un contexte d'application, $t$ un terme et $x$ une variable.
  \[ 
    x \in fv(\mathcal{L}[\lambda y.t]) \Rightarrow x \in fv(\mathcal{L}[t])
  \]
\end{lemma}
\begin{proof}
...
\begin{lemma}[Propreté des applications nettoyante]
  \label{prdbgc}
  Soient $\mathcal{L}$ un contexte d'application et $t$ un terme.
 \[
   \mathcal{L}[\lambda x.t]~\text{propre}~\Rightarrow~\mathcal{L}[t]~\text{propre}
 \]
\end{lemma}
\begin{proof}
  Par induction sur la forme de $\mathcal{L}$.
  \begin{itemize}
    ...
    \item \underbar{Cas $\mathcal{L}=\mathcal{L}_1[y/v]$~:}\\
      ...
      \begin{enumerate}
        \item $\mathcal{L}_1[\lambda x.t]~\text{propre}~\Rightarrow~ \mathcal{L}_1[t]$~propre par hypothèse d'induction.
        \item $y\in fv(\mathcal{L}_1[\lambda x.t]) \Rightarrow y \in fv(\mathcal{L}_1[t])$~par le lemme \ref{lemma0}.
        \item $v$ propre.
      \end{enumerate}
...
\begin{lemma}
  \label{lemma1}
  Soient $\mathcal{L}$ un contexte d'application, $t$ et $u$ des termes et $x$ une variable.
  \[ 
    x \in fv(\mathcal{L}[\lambda y.t]) \Rightarrow x \in fv(\mathcal{L}[t[y/u]])
  \]
\end{lemma}
\begin{proof}
  Par induction sur la forme de $\mathcal{L}$.
  \begin{itemize}
    ...
    \item \underbar{Cas $\mathcal{L}=\mathcal{L}_1[z/v]$~:}\\
      Comme $x\in fv(\mathcal{L}[\lambda y.t]) \Leftrightarrow x \in fv((\mathcal{L}_1[\lambda y.t])[z/v])$, nous avons deux cas.
      \begin{itemize}
        \item \underbar{Cas $x\in fv(v)$~:} Trivialement, $x \in fv((\mathcal{L}_1[t[y/u]])[z/v]) \Leftrightarrow x\in fv(\mathcal{L}[t[y/u]])$
        \item \underbar{Cas $x\in fv(\mathcal{L}_1[\lambda y.t])\wedge x \neq z$~:}
...
\begin{lemma}[Propreté des applications]
  ...
  Soient $\mathcal{L}$ un contexte d'application et $t$ un terme.
 \[
   x\in fv(t)~\wedge~\mathcal{L}[\lambda x.t]~v~\text{propre}~\Rightarrow~\mathcal{L}[t[x/v]]~\text{propre}
 \]
\end{lemma}
\begin{proof}
  Par induction sur la forme de $\mathcal{L}$.
  \begin{itemize}
    ...
    \item \underbar{Cas $\mathcal{L}=\mathcal{L}_1[y/u]$~:}\\
      Comme $\mathcal{L}[\lambda x.t]~\text{propre}~\Leftrightarrow (\mathcal{L}_1[\lambda x.t])[y/u]~\text{propre}$, nous pouvons en déduire trois choses.
      \begin{enumerate}
        \item $\mathcal{L}_1[\lambda x.t]~\text{propre}~\Rightarrow~ \mathcal{L}_1[t[x/v]]$~propre par hypothèse d'induction.
        \item $y\in fv(\mathcal{L}_1[\lambda x.t]) \Rightarrow y \in fv(\mathcal{L}_1[t[x/v]])$~par le lemme \ref{lemma1}.
        \item $u$ propre.
      \end{enumerate}
...
\begin{proof}
  ...
  \begin{itemize}
    \item \underbar{Cas DB~:} Lemme \ref{prdb}. 
    \item \underbar{Cas DBGC~:} Lemme \ref{prdbgc}. 
    \item \underbar{Cas LS et LSGC~:} Lemme \ref{prls}.
...
\begin{lemma}[Plongeon]
  Soient $\mathcal{C}$, $\mathcal{D}$ des contextes de substitution et $t$ un terme.
...
\begin{lemma}[Reconstruction]
  Soient $\mathcal{C}$ et $\mathcal{D}$ des contextes de substitution et $t$ un terme.
...
\begin{lemma}[Simulation raisonnable]
  Soient $t$ et $u$ des termes propres et $\mtd$ un contexte de substitution.
  \[
    t \rightarrow u~\text{avec un espace}~S \Rightarrow \etat{-,t,\mtd}_0 \rightsquigarrow^* \etat{-,u,\mtd}_2~\text{avec un espace}~2\times S+|\mtd|
  \]
\end{lemma}
