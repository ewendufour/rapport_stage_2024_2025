...
\maketitle
\begin{abstract}
Dans ce travail, nous définissons une mesure d'espace pour le LSC. 
Nous adaptons ensuite la stratégie d'appel par valeur au LSC. 
Enfin, nous montrons que cette stratégie munie de la mesure définie constitue un modèle de calcul raisonnable pour l'espace en passant par une machine abstraite.
\end{abstract}
...
\subsubsection{Propreté du LSC en appel par valeur faible}
...
% \begin{lemma}[Préservation des variables libres dans les substitutions]
%   \label{lemmapresfv}
...
\subsubsection{Comportement de FLAME}
...
\begin{proof}
  Par induction sur la forme de $\mathcal{C}$.
\end{proof}
\subsubsection{Simulation raisonnable du LSC par FLAME}
...
\subsubsection{Fonctions internes de FLAME}
